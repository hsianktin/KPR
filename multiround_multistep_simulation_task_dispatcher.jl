...
@add_arg_table! s begin
    "--profile"
        required=false
        default = "PROFILES/multistep_base.jl"
...
# @everywhere profile = "PROFILES/multistep_base.jl"
tasks = [(substrate, id, profile) for substrate in ["k", "q"] for id in 1:1000]
label = split(profile, "/")[end] |> x -> split(x, ".jl")[1]
@everywhere function execute_task(task)
    substrate, id, profile = task
    cmd = `julia multiround_multistep_simulation_single_thread.jl $substrate $id $profile`
    run(cmd)
end
...
# )
# pgfsave("DATA/survival_probability_multistep_simulation.pgf", plot1)
# pgfsave("DATA/survival_probability_multistep_simulation.pdf", plot1)
...
#     LegendEntry(L"\mathcal{I}_a"),
#     LegendEntry(L"\mathcal{C}_a"),
#     LegendEntry(L"\mathcal{I}_p"),
#     LegendEntry(L"\mathcal{C}_p"),
# )
# pgfsave("DATA/mutual_information_and_channel_capacity_multistep_simulation.pgf", plot2)
# pgfsave("DATA/mutual_information_and_channel_capacity_multistep_simulation.pdf", plot2)
...
# )
# using CSV
# CSV.write("DATA/numerical_mutual_information_and_channel_capacity_multistep.csv", df)
