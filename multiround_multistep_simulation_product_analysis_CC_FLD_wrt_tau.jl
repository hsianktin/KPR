...
k_p = 1
N_f = 6
τs = (0.2:0.2:5.0 |> collect) ∪ (0.1:0.1:1.0 |> collect)
group_label = "k_p_$(k_p)_N_f_$(N_f)"
profiles = ["PROFILES/multistep_kp_$(k_p)_τ_$(τ)_N_f_$(N_f).jl" for τ in τs]
...
@showprogress for T ∈ [1000.0]
    ...
    # obtain eps using svg
    run(`inkscape --export-type=eps FIG/multiround_multistep_simulation_product_CC_summary_$(group_label)_T_$(T).svg`)
    @info "figure saved to FIG/multiround_multistep_simulation_product_CC_summary_$(group_label)_T_$(T).pdf"
...
plt = @pgf begin
    ...
    LegendEntry("\$\\tau^*_{\\rm P}\$"),
    LegendEntry("\$\\hat \\tau^*_{\\rm P}\$"),
    LegendEntry("\$\\tau_{\\rm a}^*\$"),
...
pgfsave("FIG/multiround_multistep_simulation_product_CC_P_A_$(group_label)_grouped_plots.pdf",plt)
pgfsave("FIG/multiround_multistep_simulation_product_CC_P_A_$(group_label)_grouped_plots.pgf",plt; include_preamble=true)
pgfsave("FIG/multiround_multistep_simulation_product_CC_P_A_$(group_label)_grouped_plots.tex",plt; include_preamble=true)
pgfsave("FIG/multiround_multistep_simulation_product_CC_P_A_$(group_label)_grouped_plots.svg",plt)
@info "Grouped plots with TikZ labels saved to FIG/multiround_multistep_simulation_product_CC_P_A_$(group_label)_grouped_plots.pdf"
